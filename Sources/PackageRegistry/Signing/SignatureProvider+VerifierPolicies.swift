...
struct _OCSPVerifierPolicy: VerifierPolicy {
    ...
    >()*/
    private var cache: [UnverifiedCertificateChain: (result: PolicyEvaluationResult, expires: DispatchTime)] = [:]
...
private struct _OCSPRequester: OCSPRequester {
    ...
    func query(request: [UInt8], uri: String) async -> OCSPRequesterQueryResult {
        ...
        var httpRequest = HTTPClientRequest(url: uri)
        httpRequest.method = .POST
        httpRequest.headers = [
            "Content-Type": "application/ocsp-request",
            "Host": host,
        ]
        httpRequest.body = .bytes(request)
        do {
            let response: HTTPClientResponse = try await self.httpClient.execute(httpRequest, timeout: .seconds(30))

            guard response.status == .ok else {
                return .terminalError(SwiftOCSPRequesterError.invalidResponse(statusCode: response.status.code))
            }
            var responseBody = try await response.body.collect(upTo: 1_000_000)
            return .response(responseBody.readBytes(length: responseBody.readableBytes)!)
        } catch {
            return .terminalError(SwiftOCSPRequesterError.emptyResponse)
        }
    }
}
